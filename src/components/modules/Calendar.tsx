...
export const Calendar: React.FC = () => {
  ...
  return <Card className="glass border-border/50">
      ...
      <CardContent className="space-y-4">
        {!isReady && <p className="text-sm text-muted-foreground">Loading Google API...</p>}
        
        {error && <p className="text-sm text-muted">Error: {error}</p>}

        {!isSignedIn ? <Button onClick={() => signIn()} className="w-full" disabled={!isReady}>
            ...
            return <div key={ev.id} className="flex items-center gap-3 group">
